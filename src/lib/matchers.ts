...
import { NotraDefaultError } from "../models/errors/notra-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | NotraDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | NotraDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new NotraDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new NotraDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
