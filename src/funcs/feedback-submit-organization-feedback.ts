 ...
 */
export function feedbackSubmitOrganizationFeedback(
  client: NotraCore,
  request: operations.SubmitOrganizationFeedbackRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.SubmitOrganizationFeedbackResponse,
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        operations.SubmitOrganizationFeedbackRequest$outboundSchema,
        value,
      ),
  ...
  const pathParams = {
    organizationSlug: encodeSimple(
      "organizationSlug",
      payload.organizationSlug,
      { explode: false, charEncoding: "percent" },
    ),
  };
  const path = pathToFunc("/v1/feedback/{organizationSlug}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(202, operations.SubmitOrganizationFeedbackResponse$inboundSchema, {
      key: "Result",
    }),
