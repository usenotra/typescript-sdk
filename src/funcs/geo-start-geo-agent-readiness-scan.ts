 ...
 */
export function geoStartGEOAgentReadinessScan(
  client: NotraCore,
  request: operations.StartGeoAgentReadinessScanRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.StartGeoAgentReadinessScanResponse,
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        operations.StartGeoAgentReadinessScanRequest$outboundSchema,
        value,
      ),
  ...
  const path = pathToFunc("/v1/projects/{projectId}/geo/agent-readiness/scan")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(202, operations.StartGeoAgentReadinessScanResponse$inboundSchema, {
      key: "Result",
    }),
    M.jsonErr(
