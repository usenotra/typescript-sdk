...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.GetGeoScanRequest$outboundSchema, value),
  ...
  const pathParams = {
    ...
    scanId: encodeSimple("scanId", payload.scanId, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/v1/projects/{projectId}/geo/scans/{scanId}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.GeoScanResponse$inboundSchema),
    M.jsonErr(
